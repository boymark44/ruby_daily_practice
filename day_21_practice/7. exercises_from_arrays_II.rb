...
def double_elements(numbers_array)
    ...
    numbers_array.each do |current_num|
        doubled_elements << current_num * 2 
    end
...
def extract_long_words(string_array)
    ...
    string_array.each do |current_string|
        long_words << current_string if current_string.length > 7
    end
...
def product_of_number_and_index(numbers_array)
    ...
    numbers_array.each_with_index do |current_num, current_index|
        multiplied_numbers << current_num * current_index
    end

    rolling_sum = 0 

    multiplied_numbers.each { |current_num| rolling_sum += current_num }
...
def sum_of_string_lengths(*strings)
    ...
    strings.each { |current_char| rolling_sum += current_char.length }
