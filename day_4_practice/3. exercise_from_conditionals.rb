...
def custom_calculator(a, b, operation)
  if operation == "add"
    ...
    if b != 0
      ...
      "Cannot be divide because the denominator is Zero"
    end
  else
    "Invalid Operation Pal"
  end
end
...
def custom_calculator2(a, b, operation)
  ...
  when :divide
    ...
    b.zero? ? "Cannot be divided because of Zero denominator" : div(a, b)
  else
    "Invalid Operation"
  end
end
...
def custom_calculator3(a, b, operation)
  # 2 step validation technique:
  return "Invalid Operation" unless ["add", "subtract", "multiply", "divide"].include?(operation)
  return "Zero Division Erorr" if operation == "divide" and b.zero?
...
def divisible_by_three_and_four(num)
  # Use ternary operation to shorten the process:
...
def string_theory(string)
  string.length > 4 or string.include?("B") ? true : false
end
