...
def extract_long_words(string_array)
    new_array = string_array.select { |current_string| current_string.length > 7 }
...
def pastas_and_sauces(pasta, sauce)
    ...
    pasta.each do |pasta_type|
        sauce.each do |sauce_type|
            combinations.push("#{pasta_type.capitalize} with #{sauce_type.capitalize}.")
        end
    end
