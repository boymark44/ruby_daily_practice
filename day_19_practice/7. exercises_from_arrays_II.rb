...
# Using find:
p found_it = words.find { |word| word.include?("e") } # => "refrigerator"

# Using detect:
p found_it = words.detect { |word| word.include?("e") } # => "refrigerator"
...
def extract_long_words(string_array)
    ...
    string_array.each do |current_string|
        extracted_words << current_string unless current_string.length <= 7 
    end
...
def product_of_number_and_index(numbers_array)
    ...
    numbers_array.each_with_index do |current_num, current_index|
        doubled_elements << current_num * current_index
    end
...
def evens_and_odds(numbers_array)
    numbers_array.partition { |current_number| current_number.even? }
end
...
def sum_of_string_lengths(*strings)
    ...
    strings.each do |current_string|
        rolling_sum += current_string.length   
    end
...
def against_all_odds(array)
    array.all? { |current_number| current_number.even? }
end
