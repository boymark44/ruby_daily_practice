...
p bird_cage = birds.map { |bird| bird.length } # => [5, 7, 6, 4, 7]
puts


#* Using the collect method:
p bird_cage = birds.collect { |bird| bird.length } # => [5, 7, 6, 4, 7]
puts  
...
# Using detect:
p detected = words.detect { |word| word.include?("e") } # => "refrigerator"
...
def custom_max(array)
    ...
    greater_element = array[0]

    array.each do |current_element|
        if current_element >= greater_element   
...
def double_elements(numbers_array)
    ...
    numbers_array.each { |num| double_elements << num * 2 }
...
def product_of_number_and_index(numbers_array)
    multiplied_by_element = [] 

    numbers_array.each_with_index do |current_element, current_index| 
        multiplied_by_element << current_element * current_index
    end

    rolling_sum = 0 

    multiplied_by_element.each { |current_num| rolling_sum += current_num }
