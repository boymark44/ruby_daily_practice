...
def custom_max(array)
    ...
    array.each do |element|
        largest_element = element unless largest_element > element
    end
...
def extract_long_words(string_array)
    ...
    string_array.each do |word|
        extracted_words << word if word.length > 7
    end
...
def product_of_number_and_index(numbers_array)
    ...
    numbers_array.each_with_index do |num, index|
        multiplied_numbers << num * index
    end

    rolling_sum = 0 

    multiplied_numbers.each { |num| rolling_sum += num }
...
puts "\n\nFiltering Methods: map, select, and partition "
...
def reverse_all(string_array)
    string_array.map { |string| string.reverse }
end
...
def evens_and_odds(numbers_array)
    numbers_array.partition { |num| num % 2 == 0 }
end
