...
# find and detect - checks any array element that satisfy a condition first before others and keep track of it.
...
def using_builtin_array(array)
  ...
  return array.max if array.length != 0
end
...
def double_elements(numbers_array)
  #double_elements = numbers_array.map { |number| number * 2 }

  double_elements = numbers_array.collect { |number| number * 2 }
end
...
def extract_long_words(string_array)
  new_string_array = string_array.select { |string| string.length > 7 }
end
...
def evens_and_odds(numbers_array)
  ...
  numbers_array.partition { |number| number.even? }
end
