...
def double_elements(numbers_array)
  ...
  multiplied_by_two = numbers_array.collect { |num| num * 2 }
...
def extract_long_words(string_array)
  ...
  long_words = string_array.select { |string| string.length > 7 }
...
def product_of_number_and_index(numbers_array)
  ...
  new_array = numbers_array.each_with_index.map { |num, index| num * index }
...
def words_with_letter(string_array, letter)
  ...
  selected_strings = string_array.select { |string| string.include?(letter) }
...
def evens_and_odds(numbers_array)
  ...
  evens_add_odds = numbers_array.partition { |num| num.even? }
