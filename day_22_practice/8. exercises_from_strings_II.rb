...
def custom_index2(string, search_term)
    ...
    string.chars.each_with_index do |current_char, current_index|
        matched_search_index = string[current_index, search_term_length]

        return current_index if search_term == matched_search_index
    end
...
def longest_word1(strings)
    ...
    words.each do |current_word|
        longest_word = current_word unless current_word.length < longest_word.length  
    end
