...
def double_elements(numbers_array)
  ...
  numbers_array.each do |current_number|
    multiple_of_two << current_number * 2
  end
...
def extract_long_words(string_array)
  ...
  string_array.each do |current_string|
    longest_word << current_string if current_string.length > 7
  end
...
def product_of_number_and_index(numbers_array)
  ...
  numbers_array.each_with_index { |number, index| product_array << number * index }

  rolling_sum = 0

  product_array.each { |number| rolling_sum += number }
...
def has_greater_than_seven_characters(array)
  ...
  has_any = array.any? { |current_string| current_string.length > 7 }
...
def against_all_odds(array)
  ...
  has_all = array.all? { |current_number| current_number.even? }
