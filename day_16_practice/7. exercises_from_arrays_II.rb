...
def extract_long_words(string_array)
    ...
    more_than_seven = string_array.select { |string| string.length > 7  }
...
def product_of_number_and_index(numbers_array)
    ...
    numbers_array.each_with_index { |num, index| multiplied_by_two << num * index }

    rolling_sum = 0 

    multiplied_by_two.each { |num| rolling_sum += num }
...
def product_of_number_and_index(numbers_array)
    ...
    numbers_array.each_with_index { |num, index| rolling_sum += num * index }
...
def words_with_letter(string_array, letter)
    chosen_strings = string_array.select { |string| string.include?(letter) }
...
def evens_and_odds(numbers_array)
    evens_and_odds = numbers_array.partition { |num| num.even? }
