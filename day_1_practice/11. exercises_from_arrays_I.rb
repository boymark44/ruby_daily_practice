...
def first_and_last(string_array)
  # Use the shovel operator to append the final index to the first index:
...
def split_in_two(array)
  #* Use the revised code generated by Cody as the solution:

  # Get the length of the array:
  array_length = array.length

  # Identify the midpoint by dividing the array_length by 2.0 and invoke the round method:
  midpoint = (array_length / 2.0).round


  # To identify the first half, use the first method and provide the midpoint as argument:
  first_half = array.first(midpoint)

  # To identify the second half, use the last method and provide an argument of array_length - midpoint:
  second_half = array.last(array_length - midpoint)

  # Return the results in an array form:
