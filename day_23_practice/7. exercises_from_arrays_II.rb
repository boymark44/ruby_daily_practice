...
# Using find:
p found_word = words.find { |word| word.include?("e") } # => "refrigerator"

# Using detect:
p found_word = words.detect { |word| word.include?("e") } # => "refrigerator"
...
def extract_long_words(string_array)
    ...
    string_array.each do |string|
        array << string if string.length > 7
    end
...
def product_of_number_and_index(numbers_array)
    ...
    numbers_array.each_with_index do |num, index|
        doubled_nums << num * index
    end

    rolling_sum = 0 

    doubled_nums.each { |num| rolling_sum += num }
